...
for each in obj.dbData:
    tls.info((each.symbol, each.percent_change, each.percentage_change))
    each.percent_change = each.percentage_change
    id_ = obj.dbData.index(each)
    obj.dbData[id_]=each
    
#for each in obj.dbData:
#    tls.info((each.symbol, each.percent_change))
    
#obj.ds.datas = obj.dbData
#obj.ds.storeData()

'''

pickFrom = 'G:/pythonworkspace/ourcryptos/data/cache/20220609_cryptdata'
d = open(pickFrom,'rb')
data = pickle.load(d)
d.close()

inputlist = []
for each in data.datas:
    print(each)
